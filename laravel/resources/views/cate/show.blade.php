...
@endsection
@section('seo_title', $cate_info['seo_title'])
@section('seo_keywords', $cate_info['seo_keywords'])
@section('seo_description', $cate_info['seo_description'])
@section('content')
<!--hd结束-->
<div class="subnav">
        <div class="subnav1 wth">
            <ul>
                <li><a href="/">全部({{ $item_count }})</a></li>
                @foreach($cates as $cate)
                <li @if($cate_info->id == $cate->id)class="sbnv1"@endif><a href="/cate/{{ $cate->id }}">{{ $cate->name }}({{ $cate->count }})</a></li>
                @endforeach
            </ul>
        </div><!--subnav1 wth结束-->					
</div><!--subnav结束-->
<div class="goods">
    ...
    <div class="pages wth">
        {!! with(new \App\Foundations\Pagination\CustomerPresenter($cate_list->appends([
