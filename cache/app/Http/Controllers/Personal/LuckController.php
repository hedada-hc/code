...
class LuckController extends Controller{
	public function index(Request $request){
		...
		//echo strtotime(date("Y-m-d H:").(date("i",time()+300)).":00");
		//2016-11-30 14:09:29
		if($request->isMethod('post')) {
			...
			if($is_null->isEmpty()){
				for ($i=0; $i < 5; $i++){ 
					$data = [
						"time"=>strtotime(date("Y-m-d H:").(date("i",time())+$i+1).":00")
					];
					Luck::create($data);
				}
				return "two_ok!";
			...
			if(Luck::where('id',$id->id)->update($this->num())){
				Luck::create(["time"=>strtotime(date("Y-m-d H:").(date("i",time()+300)).":00")]);
			}
		...
		$aa = json_encode($dd);
		//paginate 获取分页
		return view("luck/index",compact('dd',"arr","aa"));
	}
	...
	public function pour($id){

		return view('luck/pour');
	}
